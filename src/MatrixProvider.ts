...
import * as sdk from 'matrix-js-sdk'; // ▶ added – tiny client for WebRTC signalling
...
export class MatrixProvider extends lifecycle.Disposable {
	...
	private readonly _onDocumentAvailable: event.Emitter<void> = this._register(new event.Emitter<void>());

	private readonly _onDocumentUnavailable: event.Emitter<void> = this._register(new event.Emitter<void>());
	...
	public readonly onReceivedEvents: event.Event<void> = this._onReceivedEvents.event;

	public readonly onDocumentUnavailable: event.Event<void> = this._onDocumentUnavailable.event;
	...
	public constructor(
		...
		// Initialize awareness if enabled
		if (this.opts.enableAwareness) {
			this.awareness = new BlockNoteAwareness(doc);
		}
	...
	private documentUpdateListener = async (update: Uint8Array, origin: any) => {
		if (origin === this || (this.webrtcProvider && origin === this.webrtcProvider)) {
	...
	private processIncomingEvents = (events: any[], shouldSendSnapshot = false) => {
		...
		const remoteMessages = events.filter(e => e.user_id !== this.matrixClient.credentials.userId);
    ...
    */
		...
		const memberReader = this._register(new MatrixMemberReader(this.matrixClient, this.reader));
		await memberReader.initialize();

		// ────────────────────────────────────────────────────────────────
		// ▶ Option A: create a tiny, dedicated Matrix client **only** for
		//             WebRTC signalling so we do not enable the heavy
		//             /sync loop on the main client.
		// ────────────────────────────────────────────────────────────────

		const webrtcClient = sdk.createClient({
			baseUrl: (this.matrixClient as any).baseUrl || (this.matrixClient as any).clientOpts?.baseUrl,
			accessToken: (this.matrixClient as any).getAccessToken ? (this.matrixClient as any).getAccessToken() : (this.matrixClient as any).credentials?.accessToken,
		...
		try {
			...
			filterId = await webrtcClient.createFilter(filter);
		} catch (_) {
			// filter creation can fail on some homeservers – continue without
		}
		...
		await new Promise<void>(resolve => webrtcClient.once('sync' as any, (state: string) => state === 'PREPARED' && resolve()));

		// Stop the tiny client when this provider is disposed
		this._register({ dispose: () => webrtcClient.stopClient() });
		...
		this.webrtcProvider = new SignedWebrtcProvider(
			this.doc,
			webrtcClient, // <- use the tiny client for signalling
			this._roomId,
			async obj => {
				await signObject(this.matrixClient, obj); // sign with main client
			},
			async obj => {
				await verifyObject(this.matrixClient, memberReader, obj, this.translator.WrappedEventType);
			},
			this.awareness,
		);
	}
	...
	private async initializeNoCatch() {
		const roomLogName: string = this.room.type === 'id' ? this.room.id : this.room.alias;
		try {
		...
		const initialLocalStateVector = Y.encodeStateVectorFromUpdate(initialLocalState);
		const deleteSetOnlyUpdate = Y.diffUpdate(initialLocalState, initialLocalStateVector);
		...
		if (arrayBuffersAreEqual(deleteSetOnlyUpdate.buffer, missingOnServer.buffer)) {
...
function snapshotContainsAllDeletes(newSnapshot: Y.Snapshot, oldSnapshot: Y.Snapshot) {
