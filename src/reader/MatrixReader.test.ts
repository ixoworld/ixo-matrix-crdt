...
// Note: This custom request handler is not needed in matrix-js-sdk v37+
function customRequestHandler(opts: any, cb: any) {
...
it("handles initial and live messages", async () => {
  ...
  try {
    ...
    reader.onEvents((msgs: any) => {
      messages.push.apply(
        messages,
        msgs.events.filter((e: any) => e.type === "m.room.message")
      );
    });
    reader.startPolling();
