...
export function initMatrixSDK() {
  // Note: Matrix.request() is no longer needed in matrix-js-sdk v37+
  // The SDK handles HTTP requests internally
}
...
export async function createMatrixUser(username: string, password: string) {
  ...
  // Note: Matrix.getRequest() is no longer available in matrix-js-sdk v37+
  // HTTP requests are handled internally by the SDK
  let sessionId = "";
  ...
  // Note: Crypto initialization is now handled automatically in matrix-js-sdk v37+
  // when startClient() is called with appropriate options
